...
$sql = "SELECT * FROM `publisher` INNER JOIN address ON publisher.fk_addressId = address.addressId ORDER BY `publisher`.`publisherId` ASC";
...
<!-- Show the results -->
    <?php
            while($row = $result->fetch_assoc()) {
                ...
                " <b>Id:</b> " . $row["publisherId"].
                " <b>Name:</b> " . $row["publName"].
                " <b>Size:</b> " . $row["publSize"].
                " <b>Address:</b> " . $row["street"] . " " . $row["houseNumb"] . "<br>" . $row["zip"] . " " . $row["city"] . "<br>" . $row["country"] .
                "</div>";
            }
            $conn->close();
        ?>
